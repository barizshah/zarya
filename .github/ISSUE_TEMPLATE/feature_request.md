---
name: 💡 Feature Request
about: Suggest an idea or improvement for Zarya
title: "[FEAT] "
labels: enhancement
assignees: barizsh
---

## 💡 Feature Summary

A clear and concise description of the feature you'd like to see.

## 🧩 Problem / Motivation

Is your feature request related to a problem? Describe it.
> e.g. I'd love to be able to...

## 🚀 Proposed Solution

Describe the solution you'd like. How should it work?

## 🔄 Alternatives Considered

Have you considered any alternative solutions or features?

## 📸 Mockups / References

If you have mockups, wireframes, or links to similar features in other apps, include them here.

## 📝 Additional Context

Any other context, links, or references about the feature request.
